...
A modular PyTorch template for regression tasks, with implemented $K$-Fold cross-validation, dynamic mini-batching via `DataLoader`, validation-monitored early stopping, and out-of-fold evaluation.

---

## Key Highlights & Methodological Best Practices

* **Reduced Data Leakage**: Feature standardisation (`StandardScaler`) is fitted exclusively on each training fold (`fit_transform`) and subsequently mapped to the validation fold (`transform`).
...
* **Mini-Batching**: Integrates `TensorDataset` and `DataLoader` with per-epoch shuffle to ensure stable stochastic gradient descent with $L_2$ weight decay regularisation
